...
path = "/run/media/anya/Seagate Backup Plus Drive/NPQO/Anritzu OSA/Data/2021-10-13/"
fbg_num = ["cav5nm_", "cav6nm_im_", "cav6nm_rev_im_",  "cav6nm_rev_", "cav8nm_rev_", "33212_", "33760_"]
...
for sn in fbg_num:
    ...
    while(idx):
        try:
            dfb = pd.read_csv(path+"T_"+sn+"bare"+str(i)+".csv")
        except:
            print("T_"+sn+"bare"+str(i)+" not existing")
            idx = False
    ...
    diff = np.abs(min(ip_avg) - max(pow_avg))
